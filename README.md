...
一个轻量、私密的 AI Assistant PWA。

## 现在有什么

- 聊天页：填好 API 后可以聊天。
- 动态页：点“写今天”，让 AI 按人设写一条纯文字日记。
- Home：直接进入 Web Search、Calendar 与 Todo，不再使用独立的 AI 手机界面。
...
Node 后端会提供网页和 `/api` 接口，并按配置运行主动消息检查。Web Search、Calendar 与 Todo 的界面和日常数据默认由前端负责。
